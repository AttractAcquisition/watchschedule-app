...
export function ScheduleHistoryButton({ data }: { data: DashboardData }) {
  ...
  const histData: DashboardData | null = useMemo(() => {
    ...
    return { schedule: { id: selected.id, start_date: selected.start_date, end_date: selected.end_date, horizon_weeks: selected.horizon_weeks, generated_at: selected.generated_at }, assignments: histAssignments, ledger: [], crew: data.crew, lanes, crewById: data.crewById }
