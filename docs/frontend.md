...
- **Department count & combinations (B2 item 6 + B5):** the picker allows **every** valid combination of the four departments, at **1..N by tier (floor of 1)** — Dual = any 1 or 2; Triple = any 1, 2, or 3 (B5 relaxed the original exactly-N). The count rule (floor 1, max N selected departments) is enforced by the Zod schema + the DB CHECK, never the engine.
- **Watch Groups (C4) — combine departments into a shared lane.** Below the department toggles, a **"Watch lanes"** group builder lets the captain assign each selected department to a **Lane** (a per-department lane selector); departments sharing a lane are **pooled into one combined rotation** (e.g. *Deck & Engineering* as a single lane). Default = each department its own lane (**groups-of-one** = today's behaviour; existing vessels render unchanged). Disjointness is structural (each department is in exactly one lane via the selector) and DB-enforced (`lane_departments` `unique(vessel_id, department)`). A live summary shows the resulting lanes. On save, the form reconciles desired groups (department-sets) against active lanes (carry unchanged / new lane for changed → **resets to even at formation, honestly** / retire+free the rest). A department in no lane simply isn't on watch (partial coverage). Persisted to `watch_lanes` + `lane_departments` (client-RW).
...
- **Crew management (CRUD):**
  ...
  - **Crew leave (C3) — the dated mechanism, alongside the toggle.** A **Leave** section (`LeaveManager`, client-RW vessel-scoped) books **dated** per-crew leave (add / cancel-soft / restore / delete). On booked leave days a crew member isn't scheduled **and** those days drop out of their fairness denominator — **standing preserved** (a few days off don't reset or count against their service). **Composition with the toggle:** the eligibility toggle is the **blanket** "off the watch bill for the whole next generation" (all dates); leave is the **dated-period** exclusion (specific dates). Both just remove the crew from the relevant eligible pools; a candidate must be *eligible* AND *not on leave that date*. The dashboard calendar marks days with crew on leave distinctly (a "🌙 N" away indicator in the week header, with a tooltip) — separate from a charter-Paused cell and from a no-eligible-crew gap.
